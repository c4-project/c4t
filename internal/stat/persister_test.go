...
func TestNewPersister(t *testing.T) {
	...
	if assert.NoError(t, s.Load(f), "should be able to load stats from file") {
		if assert.NotNil(t, s.Machines, "machines should be populated") {
			...
			assert.Equal(t, cyc.MachineID, s.Machines[mid].LastCycle.MachineID, "last cycle should be the one sent")
			assert.Equal(t, cyc.Iter, s.Machines[mid].LastCycle.Iter, "last cycle should be the one sent")
			assert.EqualValues(t, 1, s.Machines[mid].Session.FinishedCycles, "should have one finished cycle")
			assert.EqualValues(t, 1, s.Machines[mid].Total.FinishedCycles, "should have one finished cycle")

			for i := status.Ok; i <= status.Last; i++ {
				want := uint64(len(ana.ByStatus[i]))
				assert.Equal(t, want, s.Machines[mid].Total.StatusTotals[i], "status total didn't match")
				assert.Equal(t, want, s.Machines[mid].Session.StatusTotals[i], "session status total didn't match")
			}
		}
	}
